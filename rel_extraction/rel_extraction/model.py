...
def extract_triples(sub_model, obj_model, tokenizer, bert_model, text_in, id2rel, h_bar=0.5, t_bar=0.5):  #提取模型预测的关系三元组
    ...
    sub_heads_logits = (sub_heads_logits[0].squeeze(dim=1)).numpy() #1维
    #print (sub_heads_logits)
    sub_tails_logits = (sub_tails_logits[0].squeeze(dim=1)).numpy()
    ...
    if subjects:
        ...
        obj_heads_logits = obj_heads_logits[:,:-1].numpy() #删除[SEP]
        obj_tails_logits = obj_tails_logits[:,:-1].numpy()
